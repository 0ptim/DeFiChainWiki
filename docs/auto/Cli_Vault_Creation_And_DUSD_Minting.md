...
Only token DFI can be used so first of all transfer utxo to token if needed
```
...
> waitfornewblock
```
Than deposit the DFI in the vault
```
...
```
### Take DUSD loan
To take the loan use the **takeloan {"vaultId":"hex","to":"str","amounts":"str"}** command
```
...
### Loop DUSD loan
For the case you just want to profit from the negative interest rate the minted DUSD can be deposit into the wallet afterwards.
```
